	...
	for i in range(N):
		...
		if (starname in name1.values) and (starname in name2.values):
			...
			if (pd.isnull(data1['[Mn/Fe]5407'].values[idx1][0]) == False) and (pd.isnull(data1['[Mn/Fe]5420'].values[idx1][0]) == False) and (pd.isnull(data2['[Mn/Fe]5516'].values[idx2][0]) == False):
				...
				mnfeerr = np.std([mnfe5407, mnfe5420, mnfe5516])
...
def match_hires(hiresfilelist, sourcelist, obsfile, outputname):
	...
	medresMnFeerror = np.sqrt(np.power(medresMnHerror,2.) + 0.1**2.) #np.sqrt(np.power(medresFeHerror,2.) + np.power(medresMnHerror,2.))
	...
	np.savetxt(outputname, np.asarray((matched_names, matched_medres, matched_medres_err, matched_hires, matched_hires_err, separation)).T, delimiter='\t', header='name\tour [Mn/Fe]\terror(our [Mn/Fe])\thires [Mn/Fe]\terror(hires [Mn/Fe])\tseparation (arcsec)', fmt="%s")
