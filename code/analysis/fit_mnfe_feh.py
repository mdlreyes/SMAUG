...
def fit_mnfe_feh(file, mnfe_check, outfile, title, fehia, maxerror=None, nlte=False, bestfit=False, sne=False, literature=None, ia_comparison=False):
	...
	if literature is not None:
		...
		labels.append('This work (Sculptor)')
		...
		for litfile in literature:
			...
			if litfile == 'Sobeck+06':
				...
				current_label = r'Sobeck+06 (MW halo, $N='+str(len(current_feh))+' $)'
				marker = '.'
				color = 'darkgray'
				zorder = 1
			...
			if litfile == 'North+12':
				...
				current_label = r'North+12 (Sculptor, $N='+str(len(current_feh))+' $)'
				marker = 's'
				color = 'darkgreen'
				zorder = 2
		...
		# Legend
		legend = ax.legend(patches, labels, fancybox=True, framealpha=0.5, loc='best')
	...
	if bestfit:
		...
		bestfit2, = ax.plot(xfit, yfit[1], color='r', linestyle='-', linewidth=3, zorder=200)
	...
	if sne:
		...
		# Legend
		legend = ax.legend(patches, labels, fancybox=True, framealpha=0.5, loc='lower left')
		for text in legend.get_texts():
			plt.setp(text, color='k', fontsize=18)
		ax.add_artist(legend)
		'''
...
def compare_mnfe(outfile):
	...
	ax.axvline(0.27, ymin = 6./7. + dy, ymax = 7./7. - dy, color=blues[3], alpha=1, linewidth=2)
	...
	ddtl18 = [-0.17, 0.14, 0.30][::-1]
	for i in range(len(ddtl18)):
		ax.axvline(ddtl18[i], ymin = 4./7. + dy, ymax = 5./7. - dy, color=blues[i+2], alpha=1, linewidth=2)
	...
	ax.axvline(0.30, ymin = 3./7. + dy, ymax = 4./7. - dy, color='b', linestyle=':', alpha=1, linewidth=2)
