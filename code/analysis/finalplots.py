...
def plot_hires_comparison(filenames, objnames, glob, offset, sigmasys=True):
	...
	# Set up figure
	fig, (ax0, ax1, ax2) = plt.subplots(ncols=3, sharey=False, figsize=(20,7), gridspec_kw={'width_ratios':[2,1,2]})

	ax1.set_axis_off()

	# Format labels
	ax0.set_ylabel(r'$\mathrm{[Mn/Fe]}_{\mathrm{MRS}}$', fontsize=28)
	ax0.set_xlabel(r'$\mathrm{[Mn/Fe]}_{\mathrm{HRS}}$', fontsize=28)
	...
	#print('Median MRS: ',np.median(np.hstack(totalMRSerr)))
	#print('Median HRS: ',np.median(np.hstack(totalHRSerr)))
	...
	if sigmasys:
		...
		sigma_sys = np.std((totalMRS-totalHRS)/np.sqrt(np.power(totalMRSerr,2.) + np.power(totalHRSerr,2.)))
		print('Sigma_sys: ', sigma_sys)
		#ax1.fill_between([xmin, xmax], [sigma_sys, sigma_sys], [-sigma_sys, -sigma_sys], alpha=0.25, zorder=0, color='k')

	# Make legends
	l1 = ax0.legend(handles=dsphplots, bbox_to_anchor=(1.04,0.77), borderaxespad=0, fontsize=18, frameon=False, title='dSphs')
	...
	# Let's also make a histogram!

	#fig, ax = plt.subplots(figsize=(6,6))
	bins = np.linspace(-3, 3, 15)

	x = (totalMRS-totalHRS)/np.sqrt(np.power(totalMRSerr,2.) + np.power(totalHRSerr,2.))
	n, bins, _ = ax2.hist(x, bins, alpha=0.5, facecolor='None', edgecolor='k', fill=True, linewidth=1.5)

	binwidth = bins[1] - bins[0]

	# Overplot normalized Gaussian
	xbins = np.linspace(-3,3,1000)
	sigma = 1.
	mu = np.mean(x)
	print('Average: ', mu)
	y = ((1. / (np.sqrt(2 * np.pi) * sigma_sys)) * np.exp(-0.5 * (1. / sigma_sys * (xbins - mu))**2))*len(x)*binwidth

	ax2.plot(xbins, y, linestyle='-', color='r', linewidth=2)

	# Format plot
	for label in (ax2.get_xticklabels() + ax2.get_yticklabels()):
		label.set_fontsize(16)
	xlabel = r'$(\mathrm{[Mn/Fe]}_{\mathrm{MRS}}-\mathrm{[Mn/Fe]}_{\mathrm{HRS}})/\sqrt{\sigma_{\mathrm{MRS}}^2 + \sigma_{\mathrm{HRS}}^2}$'
	ax2.set_xlabel(xlabel, fontsize=20)
	ax2.set_ylabel('N', fontsize=20)
	ax2.text(1.5, 2.75, r'$\mu=$'+'{:0.3f}'.format(mu)+'\n'+'$\sigma=$'+'{:0.2f}'.format(sigma_sys), fontsize=20)
	plt.savefig('../figures/hires_comparison_test.pdf', bbox_inches='tight')
	plt.show()
...
def plot_mn_fe(filenames, outfile, title, gratings=None, maxerror=None, xlim=None, ylim=None, snr=None, solar=False, typeii=True, typei=True, averages=False, n=4):
	...
	for i in range(len(filenames)):
		...
		# Compute [Mn/Fe]
		mnfe = mnh - feh
		mnfeerr = np.sqrt(np.power(mnherr,2.) + (0.1)**2.) #np.sqrt(np.power(feherr,2.)+np.power(mnherr,2.))
		...
		if averages:
			...
			for idx in range(len(bins)):
				...
				if len(bin_match) > 1:
					...
					bindisp = np.std(mnfe_matched)
					#binerr  = np.sqrt(np.sum(np.power(mnfeerr_matched,2.)))/len(mnfeerr_matched)
					binerr = bindisp
					...
					if newlegend:
						ax.errorbar(bins[idx], np.average(mnfe_matched, weights=weights), color=cwheel[i], markersize=8, xerr=0.5*binwidth, yerr=binerr, marker='o', linestyle='', capsize=3, zorder=100, label=gratings[i]+': N='+str(len(feh)))
						newlegend = False
					else:
						ax.errorbar(bins[idx], np.average(mnfe_matched, weights=weights), color=cwheel[i], markersize=8, xerr=0.5*binwidth, yerr=binerr, marker='o', linestyle='', capsize=3, zorder=100)
...
def main():
	...
	#plot_hist(['../data/7089l1_1200B_final.csv','../data/7078l1_1200B_final.csv','../data/n5024b_1200B_final.csv'], ['M2','M15','M53'], 'error', '../figures/errorhist.pdf', membercheck=['M2','M15','M53'], memberlist='../data/gc_checks/table_catalog.dat', maxerror=0.3, sigmasys=[0.19,0.14,0.05])
